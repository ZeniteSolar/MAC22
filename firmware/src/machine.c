...
volatile contactor_t contactor;
...
inline void task_running(void)
{
    ...
    if (contactor.request_available) {
        control_contactor_coil(contactor.request);
        can_app_send_contactor_response(contactor.request);
        contactor.request_available = 0;
    }
}
...
/**
 * @brief This function control the contactor coils
 */
inline void control_contactor_coil(contactor_request_t request)
{
    switch (request) {
        case CONTACTOR_REQUEST_TURN_OFF:
            VERBOSE_MSG_MACHINE(usart_send_string("Contactor turned off.\n"));
        ...
        case CONTACTOR_REQUEST_SET_FORWARD:
            VERBOSE_MSG_MACHINE(usart_send_string("Contactor set to forward coil position.\n"));
            clr_bit(CONTACTOR_PORT, CONTACTOR_REVERSE_COIL);
            set_bit(CONTACTOR_PORT, CONTACTOR_FORWARD_COIL);

            break;
        case CONTACTOR_REQUEST_SET_REVERSE:
            VERBOSE_MSG_MACHINE(usart_send_string("Contactor set to reverse coil position.\n"));
            clr_bit(CONTACTOR_PORT, CONTACTOR_FORWARD_COIL);
            set_bit(CONTACTOR_PORT, CONTACTOR_REVERSE_COIL);
...
/**
 * @brief This function initializes the contactor coils
 */
